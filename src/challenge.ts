...
export class Challenge {
    constructor(private _id: number, private _name: string, private _routes: Set<number>, private _activity: Activities, private _total_kilometers: number, private _users: Set<string>) {
        if (_id < 0 || _id % 1 !== 0) {
            throw "ID del reto no válido";
        }
        _routes.forEach((id, index) => {
            if (id < 0 || id % 1 !== 0) {
    ...
    public addRoute(route: number | Route): boolean {
        if (typeof route === "number") {
            ...
            if (route.id >= 0 && route.id % 1 === 0 && !this._routes.has(route.id)) {
