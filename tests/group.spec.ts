...
describe("Group class test", () => {
  ...
  it("favourite_routes property", () => {
    ...
    expect(myGroup.favourite_routes).to.be.eql(new Set<number>([6, 4]));
    myGroup.favourite_routes = new Set<number>([7, 8]);
    expect(myGroup.favourite_routes).to.be.eql(new Set<number>([7, 8]));
    expect(
      () => (myGroup.favourite_routes = new Set<number>([-7, 8]))
    ...
    expect(myGroup.favourite_routes).to.be.eql(new Set<number>([7, 8, 3]));
    expect(myGroup.addFavouriteRoute(myRoute)).to.be.true;
    expect(myGroup.addFavouriteRoute(myRoute)).to.be.false;
    expect(myGroup.favourite_routes).to.be.eql(new Set<number>([7, 8, 3, 0]));
  ...
  it("historical property", () => {
    const myGroup = new Group(
      ...
      new Statistics(5, 200, 10, 500, 50, 4000),
      new Set<number>([6, 4]),
      new Set<[Date, number]>([[new Date("2022-12-14T03:24:00"), 4]])
    );
    expect(myGroup.historical).to.be.eql(
      new Set<[Date, number]>([[new Date("2022-12-14T03:24:00"), 4]])
    );
    myGroup.historical = new Set<[Date, number]>([
    ...
    expect(
      () => (myGroup.historical = new Set<[Date, number]>([[new Date(), -1]]))
    ).to.throw("ID -1 de ruta del historial no válido");
    expect(
      () => (myGroup.historical = new Set<[Date, number]>([[new Date(), 5.5]]))
