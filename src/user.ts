...
export class User {
    ...
    public addFavouriteRoute(route: number | Route): boolean {
        if (typeof route === "number") {
            ...
            if (route.id >= 0 && route.id % 1 === 0 && !this._favourite_routes.has(route.id)) {
    ...
    public addActiveChallenge(challenge: number | Challenge): boolean {
        if (typeof challenge === "number") {
            if (challenge >= 0 && challenge % 1 === 0 && !this._active_challenges.has(challenge)) {
                this._active_challenges.add(challenge);
                return true;
            }
        } else {
            if (challenge.id >= 0 && challenge.id % 1 === 0 && !this._active_challenges.has(challenge.id)) {
